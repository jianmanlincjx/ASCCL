...
def main():
    ...
    parser.add_argument('--root', type=str, default='/data2/JM/code/NED-main/MEAD_data', help='Path to MEAD database.')
    ...
    for j, actor in enumerate(args.actors):
        if os.path.exists(os.path.join(args.root, actor + '_deca.pkl')):
            ...
            for video_file in video_files:
                ...
                actor_data_all.append(params_all)
            results_pth = os.path.join(args.root, actor + '_deca.pkl')
            results_all_pth = os.path.join(args.root(), actor, '_deca_all.pkl')
            with open(results_pth, "wb") as f:
                pickle.dump(actor_data, f)
            with open(results_all_pth, "wb") as f:
                pickle.dump(actor_data_all, f)
